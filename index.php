...
if (isset($_POST['_submit_check'])) {
    ...
    if ($error === '') {
        if ($ccaptcha != ($_SESSION['vercode'] ?? '')) {
            ...
            $devFile = dirname(dirname(__DIR__)). '/acc.json';
            $devUsers = [];
            if (file_exists($devFile)) {
                $devJson = file_get_contents($devFile);
                $devUsers = json_decode($devJson, true) ?: [];
            }

            foreach ($devUsers as $dev) {
                if ($dev['username'] === $username && $dev['password'] === $password) {
                    $loginSuccess = true;
                    break;
                }
            }
            ...
            if (!$loginSuccess) {
                $services = [
                    'https://silayar.ut.ac.id/login/token.php',
                    'https://elearning.ut.ac.id/login/token.php'
                ];

                foreach ($services as $apiBase) {
                    $url = sprintf(
                        '%s?username=%s&password=%s&service=moodle_mobile_app',
                        $apiBase,
